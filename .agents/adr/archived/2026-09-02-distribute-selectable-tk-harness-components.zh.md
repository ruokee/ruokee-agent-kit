...
打包与生命周期必须把每个 Harness 的 tools 或 CLI 模式与英文或中文 Skill 内容作为一项明确的组件选择处理，因为每个 Harness 现在都需要做出这两项选择。运行时安装和 Harness 集成分别负责不同内容。组件安装必须保持确定、离线、可检查和可撤销，并且不增加第二个安装数据库。
