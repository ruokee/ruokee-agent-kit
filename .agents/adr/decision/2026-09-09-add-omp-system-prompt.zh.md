...
### 2026-09-14：追加模型级提示词规则

组件还会追加与当前 turn 模型匹配的用户自有规则文档。它读取用户与项目 agent 目录下的 `model-prompts`，并在替换步骤产出的数组之后按每个匹配文件追加一个块；替换失败时，追加步骤仍会扩展传入的宿主数组。上文的替换契约保持不变，该能力记录在 [为系统提示词扩展增加模型级规则](./2026-09-14-add-model-prompt-rules.zh.md)。
