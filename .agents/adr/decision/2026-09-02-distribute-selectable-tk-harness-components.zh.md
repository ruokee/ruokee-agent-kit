...
**通过每个组件安装运行时。** 这会产生多个所有者和可执行版本。

**使用一个四值 Skill 选项。** 分离模式与语言选项可以直接表达两个独立维度，也保留现有默认命令。

**切换前必须卸载。** 直接更新可以根据当前固定目标规划，也避免没有可用组件的中间状态。

**维护安装历史或所有权清单。** 人工修改后，历史状态可能与当前目标冲突。固定目标和当前注册状态已经足够。

**下载或接受组件来源。** 使用内嵌字节可以避免网络身份、信任和可用性合同。
