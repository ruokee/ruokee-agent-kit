...
4. 对纳入验证的每个宿主，Skill 只在显式调用时加载：普通规划、未显式调用的深入审视请求，以及对 Skill 本身的讨论都不加载它，显式调用则加载。
5. 显式调用同时支持不完整想法和已有方案，覆盖先调研再提问、问题上限与编号、记录连续性、上游回答纠正、提前结束和禁止未授权实施。行为证据记录测试的宿主、场景、结果与限制。
6. 仓库检查必须通过。
