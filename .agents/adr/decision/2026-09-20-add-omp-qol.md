...
Enabling it accepts its boundary. Within a compaction window, a matching timeout call made by unrelated work in the same process is lengthened as well, so the mechanism provides neither request isolation nor strict project isolation. It can only lengthen a deadline and never shorten one. Ambiguous ownership — an overlapping window, a window from another session, an event order that cannot be interpreted, or an owner already registered in the process — stops the adjustment for that process and reports why, instead of sharing or overriding. Disabling it restores the native function when the current value is still this component's, and reports when it is not.
