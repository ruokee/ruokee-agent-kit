...
`grill-me` is authored and independently maintained by Ruokee, consistent with the [first-party repository boundary](./2026-08-20-establish-first-party-capability-kit.md). Its questioning concept was inspired by Matt Pocock's [skills repository](https://github.com/mattpocock/skills), but this Skill was written independently rather than produced by modifying his implementation.

Distribute a self-contained English Skill under `skills/grill-me/` and a complete Chinese counterpart under `variants/zh/skills/grill-me/`. Each contains `SKILL.md` and `agents/openai.yaml`. Each language component is complete and usable on its own.
...
The Skill is available as two independently usable language components. Invocation configuration keeps sustained questioning separate from ordinary planning and review. Its behavior and host integration must satisfy the following validation requirements.
