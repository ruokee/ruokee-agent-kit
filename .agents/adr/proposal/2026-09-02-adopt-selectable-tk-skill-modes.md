# ADR proposal: Adopt selectable tk Skill modes
...
Each Skill and integration may reference only files inside its own directory, following the [self-contained distributable component decision](../decision/2026-08-24-keep-components-self-contained.md).

### Required decision changes

This proposal defines one atomic change represented by four new decisions. The existing Harness integration and component distribution decisions remain current while this proposal remains under `proposal/`.

The target repository state has these ADR changes:

1. move the [Harness integration decision](../decision/2026-08-28-integrate-tk-with-harnesses.md) and [runtime and component distribution decision](../decision/2026-08-28-distribute-tk-runtime-components.md) to `archived/`;
2. add four focused decision pairs under `decision/`:
   - `2026-09-02-add-tk-cli-only-mode`;
   - `2026-09-02-select-tk-skill-language`;
   - `2026-09-02-integrate-tk-tools-with-harnesses`;
   - `2026-09-02-distribute-selectable-tk-harness-components`;
3. add `Reverses` and `Reversed by` links between each archived decision and its replacement;
4. remove this proposal;
5. keep `proposal/.gitkeep` after removing it.

The repository-wide [language variant decision](../decision/2026-08-20-package-self-contained-skill-variants.md) remains current. It continues to govern same-name source variants. The four tk Skills use independent discovery names and therefore do not change that rule. The tk product architecture and documentation maintenance decisions also remain current and receive dated changes that record the four new decisions and the updated paths and responsibilities.
...
- The `proposal/` directory contains this authoritative bilingual pair and no other proposal.
- While this pair remains under `proposal/`, the two existing decisions remain current.
- The target repository state archives exactly the two named decision pairs and adds exactly the four named focused decision pairs under `decision/`.
...
- The target repository state removes this proposal pair and retains `proposal/.gitkeep`.
...
- Product code, bilingual public documentation, isolated Harness validation, and the four decisions describe the same completed behavior.
