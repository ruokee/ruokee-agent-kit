...
现有两项决定还要求每个 Harness 组件只安装一份英文工具模式 Skill。[Harness 集成决定](../decision/2026-08-28-integrate-tk-with-harnesses.zh.md) 固定了安装的 Skill 语言和 OMP 工具加载元数据。[运行时与组件分发决定](../decision/2026-08-28-distribute-tk-runtime-components.zh.md) 为每个 Harness 内嵌一份英文组件，并拒绝模式和语言选项。
...
每项 Skill 和集成只能引用自身目录内的文件，遵循 [保持可分发组件自包含](../decision/2026-08-24-keep-components-self-contained.zh.md)。

### 所需决策变更

本提案定义一项原子变更，最终由四项新决定表示。本提案仍位于 `proposal/` 期间，现有 Harness 集成决定和组件分发决定继续有效。

目标仓库状态包含以下 ADR 变更：

1. 将 [Harness 集成决定](../decision/2026-08-28-integrate-tk-with-harnesses.zh.md) 和 [运行时与组件分发决定](../decision/2026-08-28-distribute-tk-runtime-components.zh.md) 移动到 `archived/`；
2. 在 `decision/` 下新增四组聚焦的决定文件：
...
3. 在每项归档决定和对应的新决定之间增加 `Reverses` 与 `Reversed by` 链接；
4. 删除本提案；
5. 删除后保留 `proposal/.gitkeep`。

仓库级的 [语言 variant 决定](../decision/2026-08-20-package-self-contained-skill-variants.zh.md) 继续有效。它仍然管理同名源码 variant。四项 tk Skill 使用独立发现名称，因此不会改变该规则。tk 产品架构和文档维护决定继续有效，并增加带日期的变更，记录四项新决定及更新后的路径和职责。
...
- `proposal/` 目录只包含本提案这对权威双语文件，不包含其他提案。
- 本文件对仍位于 `proposal/` 期间，两项现有决定继续有效。
- 目标仓库状态只归档两组已命名的决定文件，并在 `decision/` 下新增四组已命名的聚焦决定文件。
- 两项替代决定与各自的归档前身相互链接；两项新增决定不归档既有决定。新增纯 CLI 模式的决定只负责该模式的 Skill 身份与行为，Skill 语言安装决定只负责 `tk install` 的语言选择合同。
- 目标仓库状态删除本提案这对双语文件，并保留 `proposal/.gitkeep`。
...
- 产品代码、双语公开文档、隔离环境中的 Harness 验证和四项决定描述相同的完整行为。
