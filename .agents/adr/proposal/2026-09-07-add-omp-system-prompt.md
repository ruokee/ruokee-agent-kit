...
Construct the replacement only after the main-block, PROJECT, and other structural checks succeed. Fill the Skill slot with either the fully normalized catalog or its complete verbatim input after a Skill fallback. Preserve array structure, relative block order, and unchanged text byte-for-byte relative to the event input. Do not promise to restore formatting that OMP already changed before emitting the event. Do not flatten the system blocks into one string or regenerate tool schemas from a different API.

Empty input stays empty. Reprocessing this extension's output must not duplicate content. Return no `systemPrompt` when there is no actual change. Unknown versions, custom prompts, malformed owned templates, missing or duplicate structural anchors, unknown target sections outside a preserved Skill catalog, and ambiguous outer boundaries leave the entire incoming array unchanged. A Skill fallback cannot authorize replacement across an unrecognized structural boundary. Failure to match descriptions inside a uniquely identified catalog does not prevent the other transformations.
...
A blanket preservation policy avoids reconstructing description spans but leaves ordinary multiline entries unnormalized even when matching metadata is available. Preserve the entire catalog when Skill matching fails, while applying the other supported transformations. Do not use preservation as a reason to skip a uniquely matched visible catalog merely because the metadata also contains hidden Skills.

### Require identical catalogs and abort all replacement on mismatch

Complete command-list equality provides a simpler correspondence check, but hidden Skills and disabled Skill commands then prevent otherwise valid static-policy replacement. This ties the core prompt change to catalog formatting. Allow unused metadata candidates and keep description-matching failures local to the Skill catalog; reserve whole-prompt fallback for structural or activation failures.
...
7. Empty input, repeated processing, consecutive turns, mode changes, and handler chaining do not duplicate content or restore stale input. Recognized owned output is a no-op both after normalization and after a Skill fallback that retained multiline descriptions. Fresh host input is evaluated independently on each turn. Unsupported versions, custom paths, and unrecognized outer structures visibly remain unmodified.
