# Agent Note: Maintain tk documentation
...
- the current English and Chinese implemented tk Agent Notes.
...
Agent Notes preserve durable decisions, alternatives, and consequences. They link to current public contracts instead of copying full command, field, or test inventories.
...
Agent Notes use the repository's fixed bilingual format and the term Harness. English and Chinese pairs move through lifecycle directories together and remain semantically aligned. The maintainer controls lifecycle changes.

### Maintenance

A public behavior change updates the owning design page, affected guide and CLI examples, both languages, affected Skill content, every current Agent Note that owns the decision, and the corresponding validation contract in the same change.
...
Readers can enter through a guide, design topic, Skill, or Agent Note and follow relative links to the owning contract. Each normative detail has one main maintenance location.
...
Final-state wording keeps public documentation compact, but review history and implementation evidence must live in Task materials or other non-public records. Agent Notes retain rationale without becoming release notes or progress logs.
