...
下面的树定义每个语言目录第一版交付的全部文件：29 篇 Markdown 文档，以及 `agents/openai.yaml` 调用策略。不交付文档子集。如果中文初稿审查后需要合并或拆分文档，先同步更新本 Note 两个语言版本中的文件树和职责表，再重新分配编号。第一版仍交付更新后文件树中的全部文档。
...
| 文档 | 内容边界 | 来源线索 |
|-|-|-|
| `SKILL.md` | 写明职责边界、导航、证据和输出约定。 | 本 Note |
