...
Keep the rule fast enough for pre-commit; a custom check that is slow or needs network access belongs in CI only. The relationship between the two gates is covered in [the pre-commit reference](skills/python-engineering/references/tooling/pre-commit.md): pre-commit is the early, fast, skippable layer, and CI is the final, complete, authoritative one.
