...
- 只读。不修改代码。
- 有具体的审查目标：一个 diff、一个文件或一组指定的文件。如果范围是整个仓库或不清晰，请用户缩小范围或明确要求完整审查。仅在用户明确提出该要求后进入完整审查。

## 步骤（Steps）

1. 建立项目事实。读取最近的 `AGENTS.md`/`CLAUDE.md`，并浏览变更周围的代码以了解现有结构和约定。
2. 扫描 diff 或指定代码。优先使用 `git diff`、`git show`、`rg` 和 `nl`，不要加载全部内容。
3. 对照以下方面检查目标：
...
4. 输出 0-5 条发现项，信号最高的排在最前。零条发现项也是有效结果。
