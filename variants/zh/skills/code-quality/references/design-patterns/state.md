...
这是状态机行为的一种特定面向对象实现，并不等同于一般的状态机。许多状态机更适合用枚举加转换表、归约函数、`match`/`case` 或分发映射来表示。在决定状态模式是否比简单实现更有价值之前，始终先阅读[状态机建模](variants/zh/skills/code-quality/references/programming-paradigms/state-machine.md)，定义状态、事件、转换、守卫和非法转换策略。
