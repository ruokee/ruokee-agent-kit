...
- **可有可无者（Dispensables）**：没有它们代码会更干净。重复代码（Duplicated Code）、死代码（Dead Code）、投机性泛化（Speculative Generality）、懒惰元素（Lazy Element，一个没有发挥应有作用的类或函数，与薄包装密切相关，参见 [thin-wrapper-function.md](./thin-wrapper-function.md)）、用于解释糟糕代码的注释（Comments）。参见 [duplicated-code.md](./duplicated-code.md)。
- **耦合者（Couplers）**：模块间过度耦合的坏味。依恋情结（Feature Envy）、不恰当的亲密关系（Inappropriate Intimacy）、消息链（Message Chains）、中间人（Middle Man）。参见 [feature-envy.md](./feature-envy.md)。
