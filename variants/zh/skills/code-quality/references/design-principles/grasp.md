...
GRASP 是一组九个启发法，用来回答最常见的设计问题：_这个职责应该放在哪里？_ 这个行为属于哪里，哪个对象应该创建那个对象，谁来协调一个用例，我们把规则放在哪里才能使变更保持局限。与 GoF 模式不同，GRASP 模式不是你要构建的结构；它们是用于良好分配职责的推理工具。它们与 SOLID（参见 [solid.md](./solid.md)）和 [tell-dont-ask.md](./tell-dont-ask.md) 自然搭配。
