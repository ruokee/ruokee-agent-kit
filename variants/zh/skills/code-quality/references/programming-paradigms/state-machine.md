...
状态机（State Machine）将过程建模为有限个命名状态、一组事件以及状态变化规则。它回答几个具体问题：系统可以处于哪些状态，接受哪些事件，哪些转换合法，如何拒绝非法转换，以及转换时运行哪些副作用。这个模型很实用，因为许多日常错误本质上都是非法状态错误。
...
- **非法转换处理（Invalid transition handling）**：当前状态不接受事件时的显式策略，例如抛出、拒绝或空操作。这必须是明确决策，不能留下静默不做任何事的未处理分支。
...
- **分发映射**：当每个事件有不同的处理逻辑时，使用 `dict[Event, Callable]`。
- **状态对象（GoF State）**：适合每个状态确实具有大量不同行为的场景，例如连接状态、编辑器模式或协议阶段。
- **库或工作流引擎**：仅在确实需要持久化、可视化或动态配置时使用。
