...
使用本技能判断代码、测试、架构、重构计划和抽象是否具备合理的结构、变更边界和维护成本。

## 进入条件（Entry Conditions）

在涉及可维护性、设计质量、测试质量或测试坏味、抽象边界、重构、代码坏味、编程范式选择、设计模式使用或设计原则时激活本技能。按信号加载参考叶子文档，不要一次性全部读取。

如果任务没有明确要求分析代码质量，即使已经加载本 `SKILL.md`，也不要继续加载 `references/` 下的任何文档。
...
| 横切行为包装调用/对象 | [装饰器（Decorator）](./references/design-patterns/decorator.md) | 外观模式、薄包装 |
...
| 辅助/包装函数未增加语义边界 | [薄包装（Thin Wrapper Function）](./references/refactoring/thin-wrapper-function.md) | KISS、外观模式 |
...
以发现项开头。原则不是机械规则，需要说明权衡。模式不是默认模板，先证明变化点确实存在。区分事实、推断、判断和建议，不要混为一谈。不要重复格式化工具或 linter 可以自动捕获的问题。

输出格式取决于模式。请遵循对应的工作流文档（`./workflow/fast-review.md`、`./workflow/full-review.md` 或 `./workflow/analysis.md`）。分析模式提供权衡和选项，而非发现项列表。
...
- 只读和分析任务不修改被审查文件。已有的报告或 Task 材料维护授权继续适用。
