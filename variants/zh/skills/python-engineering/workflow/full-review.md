...
系统性的、基于证据的 Python 工程审查。分阶段阅读、验证高严重性发现，直接报告建议。有风险的变更在执行前取得授权。仅在用户要求完整审查时运行。
...
5. 执行授权。直接报告建议，在实施以下操作前取得相应授权：
    - 不安全的修复，或修改被审查文件、创建 `.venv`/缓存、更改锁文件的命令。
    - 批量压制或全面配置更改。
    - 跨文件重构或依赖更改。
    - 行为变更。
...
- 未经明确要求修复，不要修改代码。
- 在实施不安全修复、批量编辑、跨文件重构、依赖更改或行为变更前取得授权。
