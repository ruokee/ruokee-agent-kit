...
- 每个 `session_before_compact` 信号都绑定到事件到达时生效的窗口，无论该窗口刚被创建，还是先前由自动轮次或 `session.compacting` 打开。已经中止的信号会关闭该窗口而不是被忽略；稍后中止的信号关闭它绑定的窗口。已关闭窗口遗留的监听器不能关闭后续窗口，绑定信号也不会延长守卫租期。
- 窗口在 `auto_compaction_end`、`session_compact`、会话切换、会话关闭、取消，或守卫计时器 `compactionWindowGuardMs` 到期时关闭，以先到者为准。
- 安装通过全局 symbol 中的注册表在进程级生效，注册表记录安装它的激活、包版本、激活时的 cwd 与配置快照。只有来自该激活且快照相同的安装请求才是幂等的。
- 出现以下情况时模块拒绝安装并通过日志与 `/qol` 报告原因：存在同一宿主早期补丁的标记、注册表槽位中的内容本版本无法读取、包装由其他激活持有、开关开启但当前 `AbortSignal.timeout` 已不是本模块的包装、开关关闭，或原生函数缺失。
- 同一进程中的第二次激活既不共用补丁，也不受其暗中控制。它会停止已安装的补丁：关闭打开的窗口、停止改写，并在当前值仍是本模块包装时返回原生函数。安装方激活报告 `incompatible` 与 `runtime-conflict`，新激活按自身原因保持不生效。即使总开关、本模块开关或其键使新激活处于关闭状态，该检查仍然执行；这类激活保留自身的开关状态，不报告冲突。
...
- 窗口重叠、窗口属于其他会话，或事件顺序无法识别时，该进程内的实验停用并报告原因。
...
- 包装在进程存活期间一直安装，窗口之外不起作用。关闭事件始终不到达时，守卫限制窗口保持生效的最长时间。
...
- 一个进程只有一个包装，归安装它的激活所有。第二次激活会停止该补丁并按自身原因保持不生效；被停止的激活报告 `runtime-conflict`。因此同一进程中的两个会话不会都带有该实验。
...
- **源码基线**：OMP `18.2.4`，提交 `1c0303b1f2ec515cbf4b44a9a49d68a029531aac`，源码位置如上。
- **自动检查**：OMP `18.2.4`，`bun test` 与 `tsc --noEmit`，使用记录宿主，并把 `AbortSignal.timeout` 替换为记录器，记录器把非法输入委派给原生函数。覆盖安装顺序与每种拒绝原因、幂等安装与释放、第二次激活停止已安装补丁（同 cwd、其他 cwd、本模块开关关闭、总开关关闭、键无效）、安装方激活配置快照变化、本版本无法读取的注册表槽位、读取设置抛错、设置根不是对象、未知键、总开关取值无效、改写区间、非法输入的 `TypeError` 行为、每个窗口一次提示、进程级副作用对非压缩调用方的影响、由各事件打开窗口、绑定到已打开窗口的 before-compact 信号、已经中止的信号、遗留监听器、守卫与取消与中止与切换与关闭结束窗口、重叠与外部会话拒绝，以及被替换后的还原。
- **真实 OMP CLI**：**未验证**。所需证据是真实远端请求持续超过 `300000 ms` 后成功，且后续普通模型请求仍然可用，同时记录 V1/V2 路径、尝试次数、实际耗时、取消与回退结果，以及所用的扩展组合。日志中出现 `900000` 不构成该证据。在该运行出现之前，"提高期限能让慢速远端压缩完成"没有证据，实验保持默认关闭。
