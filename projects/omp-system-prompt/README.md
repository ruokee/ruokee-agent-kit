...
- Recognizes exactly one default main block by the `<conventions>` prefix and `§ Role` identity lines, plus exactly one structurally valid PROJECT footer. Any other block passes through byte-for-byte in its original position.
...
The runtime imports are limited to two unrestricted peer dependencies: `@oh-my-pi/pi-coding-agent` for extension APIs and `@oh-my-pi/pi-utils` for the profile-aware agent directory helpers. Tests pin direct dev dependencies on `@oh-my-pi/pi-coding-agent`, `@oh-my-pi/pi-ai`, and `@oh-my-pi/pi-utils` at 18.2.3 to keep the host fixture reproducible; the dev dependency version does not restrict installation or activation.
