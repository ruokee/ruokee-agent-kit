...
OMP 18.1.11 assembles its default system prompt from blocks: a default main block, optional Computer Safety and active-repo blocks, and a PROJECT footer. On each turn the extension receives the current block array through the `before_agent_start` event and:
...
- Places Computer Use, Scratchpad, dynamic Tool I/O lines, Specialized Tools with automated QA, and AST in the runtime-modes slot. Before removing the host's fixed Tool Policy, Exploration, Workflow, Delivery, and Critical policy text, it validates the 18.1.11 required and conditional lines in their rendered order. The owned final `# Delivery` chapter is included or omitted according to `renderDelivery`.
...
- Validates the whole host Delegation section, then drops it. The owned `# Agent coordination` section contains two fixed paragraphs; the rendered concurrency cap and the extra `hub` communication hint are not carried into any owned slot. Host concurrency enforcement is unaffected, but the main policy text no longer states the current numeric cap in advance.
...
Restricted-tool and plan-mode subagents load no extensions, so the hook does not run for them. Container observation: a plan-mode parent turn carried the owned prompt, while the restricted child it spawned carried the host default prompt with the reduced `read`/`grep`/`glob`/`yield` tool set and no owned identity. No public task or agent field requests restriction directly; plan mode is the public route, and its child is the restricted child.

Handoff generation uses the base prompt; title generation and difficulty classification use their own paths. None runs this turn hook. Container observation: `/handoff` compacted the session and its side request carried the host default prompt, not the owned prompt.

Ephemeral side requests such as `/btw` do not independently run the hook; they send the live Agent prompt. Container observation: `/btw` before any turn carried the host default prompt, while `/btw` after a replaced turn carried the owned prompt, because the per-turn override stays the Agent's active prompt until the next turn replaces or clears it.

Device notifications: when an `xd://` device mounts mid-session, OMP suppresses a notice for a device the delivered base catalog already lists. A replaced prompt drops that base catalog, so the device is announced again even though the owned `### Mounted devices` slot lists it. Container observation: after `/mcp enable probe`, one provider request carried both the owned catalog entry `xd://mcp__probe_ping` and a hidden `xdev-mount-notice` for the same device; the same scenario without the extension carried the device in the host catalog and no notice. The device stayed usable in both cases. The extension does not maintain a separate device state manager, so this duplicate notice is accepted.
...
OMP runs `before_agent_start` handlers in extension installation order, and each handler receives the previous handler's output as its input. This extension reads whatever array it receives, so extensions installed after it see the owned main block instead of the default one; extensions that expect the stock 18.1.11 main block must run before it.

### Verified scope

Component checks run in the component directory: `bun run typecheck` and `bun test` (90 tests, 450 assertions). Tests render inputs at test time from the installed 18.1.11 templates and cover native tool lists, inline catalogs, Code Mode, fixed-section condition branches, misplaced condition-line rejection, one-pass slot filling, fixed-region rejection, structural boundaries, encoded installation paths, byte preservation, block order, PROJECT footer variants, Skill description normalization, hidden order...

Container checks ran in disposable CachyOS Podman containers. Each container received files with `podman cp`; no host directory was mounted, and `HostConfig.Binds` and `Mounts` were empty before destruction. Containers and copied credentials were removed.
...
- Real OMP 18.1.11 with `pro-20x/gpt-5.6-luna` accepted a configuration where the visible event catalog omitted the hidden `architect` Skill while `pi.getCommands()` still exposed it. The provider-facing instructions used the owned identity and PROJECT snapshot, retained the mounted `xd://` device entries and plugin-loaded runtime policy, and omitted the hidden candidate.
- A real OMP run with a bounded preceding catalog rewrite preserved the rewritten Skill entry byte-for-byte, applied the owned identity and PROJECT snapshot, and emitted only the local `Skill catalog formatting skipped` diagnostic.
- A manual `/skill:architect` invocation still resolved the hidden command. The provider input carried the full Skill body and user arguments in a custom message whose Provider `role` was `user`; the model replied with the requested exact marker.
...
- Device notification observed in a container: `/mcp enable probe` produced one request carrying both the owned `xd://mcp__probe_ping` catalog entry and a hidden `xdev-mount-notice` for the same device; the same scenario without the extension carried the device in the host catalog and no notice.
- Fresh OMP 18.1.11 runs with `pro-20x/gpt-5.6-luna` exercised analysis-only requests, requested prototypes, project-specific compatibility requirements, authorization boundaries, justified pauses, honest verification, quoted control tags, runtime device notices, and changed workspace context. The model performed or declined each requested action as expected, and every captured Provider request began with the owned identity.

The provider key used for these runs enforces a low concurrency limit, so some requests were answered `429` and retried. Request bodies were composed and sent before the response, so every observation above reads the final provider request content.

Remaining limits: no public task parameter, agent definition field, or configuration key requests tool restriction without plan mode, so the plan-mode child is the only reachable restricted child; the ADR records that boundary. OMP also emits the duplicate device notice only when a mount delta is pending, and the extension intentionally keeps no separate device state.
...
The runtime imports are limited to one peer dependency: `@oh-my-pi/pi-coding-agent`, pinned exactly to `18.1.11` as both peer and dev dependency. Tests also declare direct `18.1.11` dev dependencies on `@oh-my-pi/pi-ai` and `@oh-my-pi/pi-utils`; they do not rely on transitive dependency hoisting.
