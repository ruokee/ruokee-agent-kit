...
使用公开 `tk` CLI 管理项目本地的持久 Task。Task 是值得持久保存的项目内临时性努力。创建 Task 不代表已经承诺执行或完成。
...
直接运行 `tk`。需要结构化结果时使用 `--output json`。目标项目有歧义时传入 `--cwd <path>`。完整绝对 Task 路径或材料路径可以定位自己的项目。
...
普通命令可以使用全局 `--cwd` 和 `--output`。`schema generate`、`mcp` 和 version 使用各自的专用输出格式，不套用 `--output` 的统一结果结构。

## 解析 Task

精确操作接受完整 UUIDv7、绝对 Task 目录、绝对规范 `tk.toml` 或 `TASK.md`，以及项目相对 Task 路径。

名称、目录基名、UUID 前缀、文本、正则、分支和材料路径属于 search 输入。先搜索，再使用返回的规范引用。多个候选仍然都可能符合时，列出相关候选并请用户选择。
...
CLI 的 `--user-confirmed` 默认值为 true。Agent 执行命令时必须显式传入当前授权状态，不能依赖该默认值。
...
你可以编辑 `TASK.md` 正文和普通材料。受管元数据必须通过 tk 命令修改。

## 记录持久事件

事实一旦成为持久决定、纠正、已验证结论、可恢复里程碑、验证结果、已验证协作结果或阻塞，就立即运行 `tk log`，再开始另一条工作分支。

message 使用非空单行文本，细节写入 Markdown body。不要记录例行读取、命令流水、临时计划、进度百分比、未核实猜测或运行时已经记录的生命周期事件。
...
日常修改必须使用 tk 公开命令。只有 `tk.toml` 已损坏且 tk 无法表达修复时，才可以人工修复：
...
不得调用隐藏命令，也不得编辑清理清单或 Harness 配置来绕过运行时拒绝。

## 组织普通材料

只有真实 Task 需要持久文件结构时才使用普通文件。项目规则优先。
