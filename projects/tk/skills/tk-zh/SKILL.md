...
- [工具和 CLI 路由](./references/tool-use.md)：请求上下文、结果、取消和入口边界；
- [材料模式](./references/patterns.md)：可选的普通文件组织方式；
- [术语表](./GLOSSARY.md)：产品术语。

## 选择入口

search、read、create、update 和 log 使用 Harness 的 tk 逻辑工具。Pi 和 OMP 提供 `tk_search`、`tk_read`、`tk_create`、`tk_update` 和 `tk_log`，MCP Harness 可能显示带服务器前缀的对应名称。

`tk_exec` 及其 MCP 对应入口只用于 `--version`、`init`、`check` 和 `rename`。`metadata migrate`、`metadata switch`、`gc`、组件生命周期命令以及其他没有逻辑工具的支持命令，应使用公开 CLI。

逻辑工具不可用、拒绝请求或执行失败时，不得改用公开 CLI 重试同一操作。应报告集成或传输故障。不要调用隐藏命令，也不要手工编辑受管元数据、清理清单或 Harness 配置来绕过运行时拒绝。
