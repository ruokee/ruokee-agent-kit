...
create 不接受正文输入。CLI 没有 `--body` 选项，create 工具请求在顶层 Task 和子 Task item 上都把 `body` 作为未知字段拒绝。运行时创建 Task 时自动生成 `# <规范化名称>` 作为初始正文：split 模式下该标题是 `TASK.md` 的全部内容，embed 模式下该标题位于受管 frontmatter 之后。

创建之后由调用方把正文作为普通内容维护。rename 和子 Task 重试匹配不把正文作为输入或匹配条件。其余正文行为不变：运行时为 search 和 read 输出读取正文，元数据变更时逐字节保留正文，表示切换时保持正文不变。
