...
- `subtasks_dir` 是每个父 Task 下的可选相对目录。
...
子 Task 位于父 Task 目录或其 `subtasks_dir` 下，使用稳定的序号和 slug。目录拓扑表示父子关系，不在元数据中保存 `parent`。

Task 根目录没有永久索引或缓存。发现、搜索和全项目检查按需遍历文件系统。
...
候选扫描接收项目的 `metadata_mode`，并先要求目录拓扑严格合法。顶层 Task 位于 `YYYY/MM/DD-NN--slug`；每级后代位于配置的 `subtasks_dir` 下，并使用严格的 `NN--slug` 目录名。序号宽度、`--` 分隔符、合法日历日期和非空 slug 都是硬约束。

序号分配只考虑现有的规范同级 Task，不会把序号字段扩展到两位以上。序号 `99` 已占用时，创建会在写入前返回冲突。

- split 模式只把同时具有常规 `tk.toml` 和 `TASK.md` 的规范目录作为候选；
- embed 模式只把规范目录中具有严格合法 tk frontmatter 的 `TASK.md` 作为候选；
- 普通 `TASK.md`、普通 YAML frontmatter、损坏 frontmatter、字段不完整内容、不规范位置和只具有相似文件名的目录直接忽略；
- symlink、设备、目录冒充文件和越界路径不会成为候选；
- `.tk-tmp`、WAL 和保留临时目录不参与 Task 发现。

`check` 可以报告已经被项目配置明确指向、但类型或内容无效的受管载体。它不把任意相似文件升级为“损坏 Task 候选”。
...
1. 从 Task、受管文件或材料路径向上寻找第一个符合 `DD-NN--slug` 的目录。
2. 父目录必须符合 `MM`，再上一层必须符合 `YYYY`。
3. `YYYY` 的父目录是候选 Task 根。
4. 只再检查最多两个祖先目录，用于匹配项目配置和 Task 根关系。
5. 两层内无法确认项目时失败，不继续遍历到文件系统根目录。

普通上下文目录使用有界的就近项目发现。精确绝对 Task 路径和材料路径先定位其所属项目，因此调用方可以从项目 A 的 cwd 读取项目 B 的绝对引用。
...
相同请求可以在调用方确认当前状态后重新执行。每次调用都从当前规范状态重新规划。
...
rename 只修改 Task 自身的名称、目录和元数据：

1. 解析唯一 Task；
2. 规范化新名称并计算目标路径；
3. 验证目标不存在、关系有效且 Git 策略允许写入；
4. 扫描 Markdown 引用并返回列表；
5. 移动目录且不覆盖；
6. 更新元数据；
7. 追加 WAL。
...
check 严格检查项目配置、Task 载体、schema、表示一致性、路径、UUID、关系、WAL、活动操作标记和临时清单。

必要目录或文件读取失败时立即返回不完整检查，不继续收集后续诊断。完整读取后发现格式或领域错误时，检查是完整但失败。check 不修改任何内容。
