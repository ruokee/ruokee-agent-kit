...
精确绝对路径先通过规范顶层 Task 目录和有界祖先检查定位项目。随后，运行时要求该路径属于按载体发现的任务图中的 Task。Git 项目使用 Git 根。非 Git 项目不会无界遍历到文件系统根目录。

## 内存边界

发现过程构建一份内存任务图，可以使用 O(Task 数量) 内存。search 最多保留 100 个最佳结果项，`search_body=false` 时不读取正文。
