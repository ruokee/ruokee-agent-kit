...
cwd 选择顺序是请求显式值、Harness 会话目录、运行时进程目录。完整绝对任务路径和绝对材料路径可以反向定位自己的项目。
...
名称、目录基名、UUID 前缀、子字符串、正则表达式和材料路径不是精确引用。读取和修改必须解析为恰好一个任务。
...
| 字段 | 类型 | 默认值 | 合同 |
| --- | --- | --- | --- |
| `query` | 非空字符串 | 必填 | UUID、任务路径、材料路径、名称或文本 |
| `regex` | 布尔值 | `false` | 显式把普通文本解释为 Rust 正则表达式 |
| `search_body` | 布尔值 | `false` | 字符串或正则匹配时包含任务正文 |
...
结果先按匹配类别排序，同一类别按 `created_at` 降序、ID 升序排列。每项包括任务摘要、`match`、`closed_ancestors` 和精确任务引用。`match` 至少为 `uuid`、`path`、`regex` 或 `string`。

发现任务图可以包含项目中的全部有效任务。search 最多保留前 100 个结果项。无效标记载体和相似普通文件不会作为任务返回。必要发现 I/O 失败或资源上限错误会使搜索失败，不返回部分任务图。
...
`minimal` 返回元数据和受管路径，不读取任务正文或 WAL。`summary` 返回完整的当前正文，以及包含 `timestamp`、`actor` 和 `message` 的最近 WAL 条目。`detailed` 返回相同的任务信息，并增加存在的 WAL `body`。
...
运行时在创建任务时自动生成 `# <规范化名称>` 作为初始 `TASK.md` 正文，并且不接受任何 create 请求中的正文输入。调用方在创建完成后用普通文件操作写入 `TASK.md`。
...
| --- | --- |
| `type` | `subtasks` |
| `parent_ref` | 精确、非 closed 父任务 |
| `subtasks` | 1 到 50 项，每项携带 `name`、可选 `status`（默认 `open`）、可选 `created_at`、`depends_on`、`related_to` 和 `extra`；item 不接受正文字段 |
| `user_confirmed` | 当前对话是否明确授权创建 planning 子任务，默认 `false` |
| `cwd` | 项目解析 |

创建 open 子任务不需要新的顶层授权。批次只要包含 planning 子任务，就要求 `user_confirmed=true`。首次写入前完成全部校验。新子任务在配置的创建目录下使用 `NN--slug`。编号统计所有叶子目录使用生成式名称的已发现直接子任务，取最大序号且不填补空洞，最大为 `99`。部分创建失败时返回已创建和未创建项。重试只把已发现直接子任务与 create 拥有的请求字段（`name`、`status`、显式提供的 `created_at`、`depends_on`、`related_to`、`extra`）匹配；正文不参与匹配。
...
log 请求包含 `task_ref`、非空单行 `message`、可选 Markdown `body`、可选 `actor` 和 `cwd`。它只向非 closed 任务追加一条 WAL 事件，不修改元数据。
...
rename 结果包含原始旧名称、存在时的已解析父任务路径、旧路径、规范化后的新名称、目标路径，以及每个 Markdown 引用及其行号。完整 UUID、精确任务目录和精确受管载体可以定位旧字符串名称或可识别生成式后缀需要修复的候选；closed 任务同样适用，并保持其状态。其他校验保持严格。非生成式子任务目录保持不变；生成式子任务和顶层路径保留序号并更新 slug。

exec 的 rename 与 CLI 采用相同的断链处理。执行会移动任务路径且存在旧路径引用时，在首次写入前以 conflict 错误停止。在 argv 中传入 `--ignore-brokenlinks` 可以继续移动；引用文件保持原样，结果中仍会报告它们。dry-run 在生成合法计划后始终成功。
